...
function codegenOutputTypesForEvents(outputs) {
    return `{
    ${outputs.map((param) => (param.name ? `${param.name} : ${types_1.codegenOutputType(param.type)}, ` : '')).join('\n')}
    ${outputs.map((param, index) => index.toString() + ':' + types_1.codegenOutputType(param.type)).join(', ')}
  }`;
}
